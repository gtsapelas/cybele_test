...
def clean(request, pk=None):
    ...
    try:
        ...
        # TO BE CHANGED!!
        time_start_timestamp = int(datetime.timestamp(datetime.now() -  timedelta(days=1000))) * 1000

    date_now = datetime.now()
    time_end_timestamp = max([d.temporalCoverageEndTimestamp for d in Dataset.objects.all() if d.temporalCoverageEndTimestamp != ""] + [int(time.mktime(date_now.timetuple())) * 1000])
...
def save_query(request, pk=None, temp=1):
    ...
    if not pk:
        ...
        if current_user.is_authenticated:
            ...
            if int(temp) == 1:
                # q = TempQuery(user=User.objects.get(username='Cybele'))
                q = TempQuery()
            else:
                # q = Query(user=User.objects.get(username='Cybele'))
                q = Query()
    else:
        # print 'not pk'
