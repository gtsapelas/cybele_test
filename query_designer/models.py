...
class AbstractQuery(Model):
    user = ForeignKey(User, related_name='queries', on_delete=CASCADE, null=True, default=None)
                        ...
                        if x['name'] == filters:
                            if x['type'] != 'VALUE':
                                ...
                                if v_obj.dataset.stored_at == 'UBITECH_POSTGRES' or \
                                        v_obj.dataset.stored_at == 'UBITECH_PRESTO' or \
                                        v_obj.dataset.stored_at == 'CYBELE_LXS':
                                    col_name = v_obj.name
                                else:
                                    col_name = 'value'
                    if use_table_names:
    ...
    def process(self, dimension_values='', variable='', only_headers=False, commit=True, execute=False, raw_query=False, from_visualizer=False):
        ...
        try:
            ...
            pass
        try:
            is_lxs = 'LXS' in Variable.objects.get(pk=self.document['from'][0]['type']).dataset.stored_at
        except IndexError:
            pass
...
class Formula(Model):
    ...
    created_by = ForeignKey(User, blank=True, null=True, default=None, on_delete=CASCADE)
...
class TempQuery(AbstractQuery):
    original = ForeignKey(Query, null=True, on_delete=CASCADE)
